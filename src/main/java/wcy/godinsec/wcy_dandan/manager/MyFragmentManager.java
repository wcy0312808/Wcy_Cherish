...
public class MyFragmentManager implements OnManagerFragmentListener {
    ...
    public  void onSwitchFragment(Class fragmentClass) {
        ...
        if (showFragment == null) {   //首次显示，先创建实例
            try {
                showFragment = (Fragment) fragmentClass.newInstance();                               //根据类名重新实例化Fragment，避免了实例化Fragment
                if (showFragment == null || showFragment == mCurrentFragment)
                    return; //想要展示页 == 当前展示页 不做处理

                //首次依附fragment判断
                if (mCurrentFragment == null) {
                    //该页面第一次依附fragment，直接添加
                    mFragmentManager.beginTransaction()
                            .add(R.id.fl_content, showFragment, showFragment.getClass().getName())
